    ...
    public void handleRollFile(@NotNull SourceFile fileDescriptor) {
        ...
        for (int retryAttempts = 0; true; retryAttempts++) {
            try {
                ...
                String errorContext = String.format("IngestionClientException when ingesting data into KustoDB, file: " +
                                "%s, database: %s, table: %s, operationId: %s",
                        fileDescriptor.path, ingestionProps.ingestionProperties.getDatabaseName(),
                        ingestionProps.ingestionProperties.getTableName(),
                        ingestionProps.ingestionProperties.getIngestionMapping().getIngestionMappingReference());
                throw new ConnectException(errorContext,exception);
            }
        }
    }
