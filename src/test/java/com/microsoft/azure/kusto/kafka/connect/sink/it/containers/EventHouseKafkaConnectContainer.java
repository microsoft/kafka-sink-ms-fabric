...
public class EventHouseKafkaConnectContainer extends GenericContainer<EventHouseKafkaConnectContainer> {
    ...
    public EventHouseKafkaConnectContainer(final String version) {
        super(KAFKA_CONNECT_IMAGE + ":" + version);
        waitingFor(Wait.forHttp("/connectors").forStatusCode(200));
        withExposedPorts(KAFKA_CONNECT_PORT);
    }

    public EventHouseKafkaConnectContainer withKafka(final @NotNull KafkaContainer kafkaContainer) {
        return withKafka(kafkaContainer.getNetwork(), kafkaContainer.getNetworkAliases().get(0) + ":9092");
    }

    public EventHouseKafkaConnectContainer withKafka(final Network network, final String bootstrapServers) {
        withNetwork(network);
        ...
        env.put("CONNECT_PLUGIN_PATH", "/kafka/connect;/kafka/connect/kafka-sink-ms-fabric");
        withEnv(env);
        return self();
    }
