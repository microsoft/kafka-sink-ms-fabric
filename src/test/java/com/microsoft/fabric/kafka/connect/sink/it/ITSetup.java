...
public class ITSetup {
    static @NotNull ITCoordinates getConnectorProperties() {
        ...
        String kqlDbConnectionString = getProperty("kqldbconnectionstring", "e2e", true);
        String esConnectionString = getProperty("esconnectionstring", "e2e", true);
        String esHub = getProperty("eshub", "e2e", true);
        String defaultTable = testPrefix + UUID.randomUUID().toString().replace('-', '_');
        String table = getProperty("table", defaultTable, true);
        return new ITCoordinates(getAccessToken(), database, table, kqlDbConnectionString, esConnectionString, esHub);
    }

    private static String getAccessToken() {
        String clusterScope = String.format("%s/.default", "https://kusto.kusto.windows.net");
